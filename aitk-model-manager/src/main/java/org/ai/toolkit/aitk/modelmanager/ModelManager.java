...
public class ModelManager implements InitializingBean {
    ...
    private static final Map<String, ModelInfo> ONLINE_MODEL_MAPPING = new ConcurrentHashMap<>();
    ...
    public void loadModel(String modelId, Device device) {
        Criteria criteria = ALL_MODEL_MAPPING.get(modelId).getCriteria();
        ModelDefinition modelDefinition = ALL_MODEL_MAPPING.get(modelId);
        ModelInfo modelInfo = new ModelInfo(modelId, "", criteria);
        try {
            Field engineNameField = ModelInfo.class.getDeclaredField("engineName");
            engineNameField.setAccessible(true);
            engineNameField.set(modelInfo, modelDefinition.getEngine().name());
            ...
            ONLINE_MODEL_MAPPING.put(modelId, modelInfo);
        } catch (Exception e) {
    ...
    public void unloadModel(String modelId) {
        ...
        ModelInfo modelInfo = ONLINE_MODEL_MAPPING.get(modelId);
